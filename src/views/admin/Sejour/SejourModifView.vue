<template>
    <div v-if="ready" class="text-white">
        <div class="flex mb-6 justify-between">
            ...
            <h1 class="text-2xl text-center font-bold text-white">Modification du séjour n°{{ sejour.idSejour }}</h1>
            <p @click="popitup(`/sejour/${sejour.idSejour}-view`, sejour.titreSejour)" class="cursor-pointer bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded" title="Cliquer ici pour avoir un aperçu du séjour que vous modifier">
                Aperçu
            </p>
        </div>
        ...
        <form @submit="addNewActivite">
            <div class="mb-6 flex gap-6 w-full items-center">
                ...
                <div class="flex w-1/2 gap-6 " v-auto-animate>
                    <label class="inline-flex relative items-center cursor-pointer">
                        ...
                        <span class="ml-3 text-center text-sm font-medium text-gray-300">{{ libelleTemps ? 'Avec ' : 'Sans ' }}libellé temps</span>
                    </label>
                    <div v-if="!libelleTemps" class=" w-2/5">
                        <label for="nbNuits" class="block mb-2 text-sm font-medium text-white">Nombre de nuits</label>
            ...
            <p for="visitors" class="block mb-2 text-sm font-medium text-white">Photo du séjour</p>
        ...
        <Popup title="Ajouter/Supprimer un hébergement" :show="showHebergementPopup" @update:show="showHebergementPopup = $event" @submit="submitHandler" v-auto-animate>
            <div v-if="hebergementStep == 1">
                ...
                <ul class="flex gap-3 justify-center items-center h-fit">
                    <li v-for="etape in sejour.etapeSejourNavigation" :key="etape.idEtape" class="max-w-sm h-full bg-white border border-gray-200 rounded-lg shadow-md dark:bg-gray-800 dark:border-gray-700">
                        <img class="rounded-t-lg w-full h-auto" :src="etape.photoEtape" alt="" />
                        <div class="p-5">
                            <a href="#">
                                <h5 class="mb-2 text-2xl font-bold tracking-tight text-white ">{{ etape.titreSejourEtape }} <Tooltip :text="etape.descriptionSejourEtape"/></h5>
                            </a>
                            <a @click="idEtapeHebergement = [etape.idEtape, etape.idHebergement], hebergementStep = 2" class="inline-flex mr-3 items-center px-3 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                                Choisir {{ [etape.idEtape, etape.idHebergement] }}
...
const openHebergementHandler = () => {
    if(AllHebergement.value == null) {
        apis.getHebergements().then((response) => {
            AllHebergement.value = response.data;
        });
    }
...
const getSejour = async () => {
    ...
    form.value.idSejour = sejour.value.idSejour;

    themes.value = (await apis.getThemes()).data;
    destinations.value = (await apis.getDestinations()).data;
...
const addNewActivite = async (event) => {
    ...
    await apis.uploadImage(formData).then((response) => {
        ...
        form.value.photoSejour = response.data
    }).catch((error) => {
        console.log(error);
        return;
    });
